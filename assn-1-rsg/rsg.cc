...
static void printRsg(string non_term, map<string, Definition>& grammar) {
  // map<string, Definition>::iterator curr = grammar.begin();
  assert(grammar.find(non_term) == grammar.end());
  Production prod = grammar[non_term].getRandomProduction();
  Production::iterator curr = prod.begin();
  if (!(prod.end() == prod.begin())) {
    while(curr != prod.end()) {
      //iterate over the production using iterator
      if ((*curr)[0] == '<') {
	printRsg(*curr, grammar);
      }
      else {
	cout << *curr << " ";
      }
...
int main(int argc, char *argv[])
{
  ...
  //print three rsg sentences
  for(int i = 1; i <= 3; ++i) {
    cout << "Version #" << i << ": -------------------" << endl;
    string start = "<start>";
    printRsg(start, grammar);
    cout << endl;
  }
  return 0;
}
