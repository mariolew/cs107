...
#include "wordcount.h"
static void printArticles(vector *found, const char *word);
static void loadStopWords(hashset *s);
static void Welcome(const char *welcomeTextFileName);
static void BuildIndices(const char *feedsFileName, hashset *stopWords, hashset *prevSeenArticles, hashset *wordCounts);
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle, const char *unused, const char *articleURL,
                         hashset *stopWords, hashset *wordCounts);
static void QueryIndices(hashset *stopWords, hashset *wordCounts);
...
static const char *const kStopListFile = "/usr/class/cs107/assignments/assn-4-rss-news-search-data/stop-words.txt";
...
/**
 * Print Function: printArticles
 *------------------------------
 * print article info that was found
 */

static void printArticles(vector *found, const char *word) {
    int len = VectorLength(found);
    len = len < 10? len : 10;
    printf("Nice! We found %d articles that include the word \"%s\"\n\n", len, word);
    for (int i = 0; i < len; i++) {
        articleCount *ac = VectorNth(found, i);
        printf("\t%d.)\t\"%s\" [search term occurs %d times]\n", i + 1, ac->news.title, ac->occur);
        printf("\t\t\"%s\"\n", ac->news.url);
    }
}
